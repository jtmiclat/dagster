...
class ResourceDefinition(object):
    '''Resources are scoped ways to make external resources (like database connections)
    available to solids during pipeline execution and to clean up after execution resolves.
    ...
    Args:
        resource_fn (Callable[[InitResourceContext], Any]): User-provided function to instantiate
            the resource, which will be made available to solid executions on the
            ``context.resources`` object.
        config_field (Optional[Field]): The type for the configuration data for this resource, which will be
            available to the ``resource_fn`` as ``init_context.resource_config``.
...
def resource(config=None, config_field=None, description=None):
    ...
    the resource (like the ``resource_fn`` argument to a :py:class:`ResourceDefinition`.
